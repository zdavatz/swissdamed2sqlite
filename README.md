...
- **MiGeL** — matches UDI devices against MiGeL (Mittel- und Gegenständeliste) codes (~15,700 of ~203,500 rows, category-level precision ~99.9%) in two layers:
...
![MiGeL Matching Stats](png/swissdamed_migel_stats_17h29.14.09.2026.png)

### Triage Status Sheet

Public-vs-professional / shop-sellability status of the swissdamed corpus — see the latest sheet: [**swissdamed_triage_status_14.09.2026.pdf**](pdf/swissdamed_triage_status_14.09.2026.pdf) (`--status-pdf`, two pages: status + nomenclature legend with source links). All figures are pulled live from the newest `udi_details_*.db`; each distribution share also shows its change in percentage points (`±X.XX Pp.`, two decimals, rendered smaller) versus the previous day's DB.
