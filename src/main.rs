...
fn extract_pem_from_p12(p12_path: &str) -> Result<String, Box<dyn std::error::Error>> {
    let output = Command::new("openssl")
        .args(["pkcs12", "-in", p12_path, "-nocerts", "-nodes", "-passin", "pass:notasecret", "-legacy"])
        .output();
    ...
    let output = match output {
        ...
        _ => {
            Command::new("openssl")
                .args(["pkcs12", "-in", p12_path, "-nocerts", "-nodes", "-passin", "pass:notasecret"])
                .output()?
        }
    };
...
fn gdrive_upload_csv(args: &Args, csv_path: &str) -> Result<(), Box<dyn std::error::Error>> {
...
fn get_gmail_access_token(
    ...
    body.get("access_token")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
        .ok_or_else(|| format!("No access_token in Gmail response: {}", body).into())
}
...
fn send_email_with_attachment(
    ...
    let sub_email = args.gdrive_sub.as_deref()
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(ref chrn) = args.lookup_chrn {
        return run_lookup_chrn(chrn, &args);
    }
