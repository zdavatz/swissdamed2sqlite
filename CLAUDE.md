...
Two-layer pipeline reaching ~11,400 matches from ~62,000 rows (~18%):
...
**Layer 2 — Heuristic matcher** (shared with fb2sqlite). Adds ~3,550 matches for non-SIGVARIS manufacturers via Aho-Corasick. Key features:
...
- **English-to-German enrichment**: ~80 medical terms translated (e.g., "knee" → "knie knieorthese", "nebulizer" → "vernebler aerosol", "scoli" → "skoliose rumpf orthesen"); context-aware: "ortho" + "rehab" → "spezialschuhe". Region-gated recall blocks: compression `garment`→Leib/Rumpf 05.11 (body) / 17.15.03 (arm) / 17.15.05 (face); `ostomy`→Stoma-/Fistelversorgung 29.01; `urine`+`bag`→Bein-/Bettbeutel 15.14/15.15; `superabsorbent`/`gelling`→35.05.05/.07; `Halskrawatte`→Cervikalstütze; `Gilchrist`→Schultergürtel-Orthese 22.09.
- **Category hierarchy keywords** from MiGeL XLSX parent categories
- **Per-language scoring**: DE (suffix + fuzzy >= 6 chars + compound decomposition), FR/IT (exact word only). `COMPOUND_PREFIXES` includes body-part prefixes (knie/ellenbogen/sprunggelenk/unterschenkel/finger) so German one-word compounds like `Knieschiene`→`knie`+`schiene` match the body-part keyword.
- **Precision filters**: stop words (incl. generic FR company tokens `fabrication`/`medicaux`/`produits`/`conception` that otherwise leak in via the appended company name), universal exclusions (PTA/stent/ERCP/surgical gloves, AGFA imaging, CSF/ventricular catheters), ~60 negative keywords per MiGeL code prefix incl. chapter-wide rules (catheters / blood-pressure monitors ∉ orthosis chapters 22/23), company exclusions.
- **Thresholds**: 2+ keywords: score >= 0.3, max len >= 6; single keyword: score >= 0.5, len >= 8 (>= 0.7 for verbose)
- swissdamed-specific company exclusions live in `excluded_companies` in **both** `src/reports.rs` AND `src/gui.rs` (keep in sync): Varian (radiation therapy), Sunstar (dental), Diacor (transfer furniture), SOMNOmedics (sleep-lab PSG sensors), Accuratus (surgical instruments), ATMOS (suction/ENT), CONCEPTION ET FABRICATION (dental), iNOsystems (NO delivery) — each a pure non-MiGeL manufacturer whose entire matched output would otherwise be false positives.
- Key matches: Macom/LymphCare compression garments (~650), Künzli shoes (464), Aspen orthoses (272), Achim Ruthner German orthoses (Knieschiene/Ellenbogenschiene/Halskrawatte/Gilchrist), Huizhou Foryou foam + superabsorber + gelling-fiber dressings, Salts ostomy, Guido Buschmeier infusion sets/stands, PRIM, Angelini ThermaCare, O2 concentrators, nebulizers, condoms, prosthetics.
