...
- `src/triage.rs` — `--triage` mode + `classify()`: labels each `udi_details` row `intendedUser` ∈ {professional, public, review} + `iuConfidence` {high/medium/low} + `iuReason`. **Decision-support, NOT a compliance determination.** swissdamed exposes no structured intended-user field for MDR (95% of corpus); only IVD carries `selfTesting`/`nearPatientTesting`/`professionalTesting`. Deliberately asymmetric — `public` only from a structured IVD self-testing signal or an explicit lay-use text statement (a false `public` = selling a professional-only device = breach; a false `professional` = only a lost sale). Rule order (first match wins): (1) professional/high = AIMDD ∪ implantable ∪ CLASS_III/D; (2) public/high = IVD selfTesting ∪ risk class IVD_DEVICES_SELF_TESTING; (3) professional/medium = IVD professional/near-patient testing; (4) medium text phrases (EN/DE/FR/IT prof/lay); (5) professional/low = EMDN professional categories `EMDN_PROFESSIONAL_CATS` = L/K/C/G/H/J/S (surgical/interventional/sterilisation — exclusion-only, regulatory review of the list recommended; F deliberately excluded — home dialysis); else review/low with an `EMDN_PUBLIC_LEAN_CATS` = Y/N (orthoses/TENS) lean note. Measured on 152,974 rows: professional 64,159 (42%) · public 61 · review 88,754 (58%). Risk class & EMDN proven NOT to identify public; free text yields <1% reliable signal. Integrated into `--details` (`classify()` appends the 3 columns during a fresh run); `--triage` re-classifies an existing DB in place (ADD COLUMN + UPDATE, idempotent, re-parses `rawJson` so it stays the single source of truth). Every `review` row (and any listed product) must be verified against the manufacturer IFU/Zweckbestimmung + MepV Abgabe rules before use.
