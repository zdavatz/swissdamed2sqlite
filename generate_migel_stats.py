...
company_colors = [
    ...
    '#c8e6c9', '#e8f5e9', '#f1f8e9', '#f9fbe7', '#fffde7',
    '#fff9c4',
]
...
metrics = [
    (f'{total_matched:,}', 'MiGeL matched products'),
...
for i, (value, label) in enumerate(metrics):
    y = 0.85 - i * 0.22
    ax1.text(0.05, y, value, fontsize=24, fontweight='bold',
             color=accent, transform=ax1.transAxes, va='center')
    ax1.text(0.40, y, label, fontsize=13, color=text_color,
...
# Shorten long company names for legend
def short_name(name):
    if len(name) > 25:
        return name[:22] + '...'
    return name
...
ax2.set_title('Matches by Company', fontsize=14, fontweight='bold',
...
bars = ax3.barh(bar_positions, migel_values[::-1],
                color=bar_color, edgecolor='#ffffff', height=0.7, alpha=0.85)
...
ax3.set_title('Top MiGeL Categories', fontsize=14, fontweight='bold',
              color=title_color, pad=12)
ax3.set_yticks([])
